...
function Get-PrinterData {
    ...
    try {
        ...
        if (-not $modelResp.Success) {
            Write-Log "[$TargetHost] Failed to get printer model" -level "WARNING"
            return $null
        }
        ...
        if (-not $nameResp.Success) {
            Write-Log "[$TargetHost] Failed to get printer name" -level "WARNING"
            return $null
        }
        ...
        if (-not $oidSet) {
            Write-Log "[$TargetHost] No OID set for model: $model" -level "WARNING"
            return $null
        }
        ...
        function Resolve-SnmpValue {
            ...
            if ($Name -in @("Display", "Status")) {
                try {
                    if ($GetBulkWalk) {
                        ...
                        Write-Log "[$TargetHost] Display/Status requires GetBulkWalk=true or regular SNMP GET" -level "BULK"
                        return ""
                    }
                }
                catch { 
            ...
            if (-not $ValueObj.Success) {
                Write-Log "[$TargetHost] SNMP error for $Name" -level "WARNING"
                return "Error"
            }
            ...
            switch -CaseSensitive ($Name) {
                ...
                "Serial" {
                    try { return ($data.ToString().Replace('??','') -split '-')[0] }
                    catch { return "Error" }
                }
        ...
        foreach ($item in $oidSet.GetEnumerator()) {
            ...
            try {
                ...
                Write-Log "[$TargetHost] Exception in Get-SnmpData for $Name : $($_.Exception.Message)" -level "WARNING"
                $results[$Name] = "Error"
                continue
            }
        }
        ...
        Write-Log "[$TargetHost] Fatal error in Get-PrinterData: $($_.Exception.Message)" -level "ERROR"
        return $null
    }
}
...
function Invoke-PrinterWorker {
    ...
    while ($true) {
        ...
        try {
            Write-Log "Starting SNMP for $ip" -level "INFO"
            ...
            if ($PrinterData) {
                ...
                $Results.Add([PSCustomObject]@{
                    ...
                    Error       = $null
                    Index       = $idx
                    SnmpErrors   = $printerErrors.ToArray()
                })
            }
            else {
                $Results.Add([PSCustomObject]@{
...
function Get-PrintersDataParallel {
    ...
    if ($online.Count -eq 0) {
        Write-Log "No online printers to check" -level "WARNING"
    ...
    if ($completed -ne $total) {
        Write-Log "Only $completed out of $total hosts are successful" -level "WARNING"
    }
