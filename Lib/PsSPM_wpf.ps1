...
try {
    ...
    Write-Host "Ошибка PresentationFramework: $_" -ForegroundColor Red
}
try {
    Add-Type -AssemblyName PresentationCore -ErrorAction Stop
} catch {
    Write-Host "Ошибка PresentationCore: $_" -ForegroundColor Red
}try {
    Add-Type -AssemblyName WindowsBase -ErrorAction Stop
} catch {
    Write-Host "Ошибка WindowsBase: $_" -ForegroundColor Red
}
try {
    Add-Type -AssemblyName System.Windows.Forms -ErrorAction Stop
} catch {
    Write-Host "Ошибка System.Windows.Forms: $_" -ForegroundColor Red
}
...
function Show-UserGUIXaml {
    ...
    $BulkCheckBox.isChecked = $script:GetBulkWalk
    $BulkCheckBox.Add_Checked({ $script:GetBulkWalk = $true })
    $BulkCheckBox.Add_Unchecked({ $script:GetBulkWalk = $false })
    ...
    $ButtonRun.Add_Click({
        ...
        $script:SNMPThreads = $xSNMPThreads.Text
    ...
    if ($script:PrinterRange) { return $script:PrinterRange }
    elseif ($script:selectedFile) { return $script:selectedFile }
    else { return $null }
}