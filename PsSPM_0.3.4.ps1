...
try {
    Write-Log "=== Starting Printer Monitoring Version: 0.3.4 ==="
    ...
    if (-not $ShowGUI) {
        ...
        if (-not ($selectedFile = Show-UserGUIXaml -Directory $PrinterListPath)) {
            Write-Log "Printer list file not load" -Level "ERROR"
            throw "Printer list file not load" | Out-Null
        } else { Write-Log "=== Printer list: OK ===" }
    }
    ...
    elseif (-not ($printers = Import-Csv -Path $selectedFile -Header Value)) {
        Write-Log "=== Import Printers IP: Error ===" -Level "ERROR"
        throw "Import Printers IP: Error" | Out-Null
    } else { Write-Log "=== Import Printers IP: OK ===" }
